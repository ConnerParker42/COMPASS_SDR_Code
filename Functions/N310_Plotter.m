...
%   Plots: Graph of phase data
    ...
    if (isTableCol(parsedData{1}, "In Phase [V]"))
        ...
        fprintf('Power Diff between A0 and A1: %d dB\n', 20*log10(max(signal1)) -20*log10(max(signal2)))
        fprintf('Power Diff between B0 and B1: %d dB\n', 20*log10(max(signal3)) -20*log10(max(signal4)))
    end
